...
class gateway extends \core_sms\gateway {
    ...
    public function send(
        ...
        $awsconfig = $DB->get_field(
            table: 'sms_gateways',
            return: 'config',
            conditions: ['id' => $message->gatewayid, 'enabled' => 1, 'gateway' => 'block_coursesentimentgateway_aws\gateway',],
        );
        $status = \core_sms\message_status::GATEWAY_NOT_AVAILABLE;
        if ($awsconfig) {
            $config = (object)json_decode($awsconfig, true, 512, JSON_THROW_ON_ERROR);
            $class = '\block_coursesentimentgateway_aws\local\service\\' . $config->gateway;
