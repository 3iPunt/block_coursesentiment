...
namespace block_coursesentimentgateway_aws;
 ...
 *
 * @package    block_coursesentimentgateway_aws
...
class hook_listener {
    ...
    public static function set_form_definition_for_aws_sms_gateway(after_sms_gateway_form_hook $hook): void {
        if ($hook->plugin !== 'block_coursesentimentgateway_aws') {
            return;
        }

        $mform = $hook->mform;

        $mform->addElement('static', 'information', get_string('aws_information', 'block_coursesentimentgateway_aws'));

        $gateways = [
            'aws_sns' => get_string('aws_sns', 'block_coursesentimentgateway_aws'),
        ];
        $mform->addElement(
            'select',
            'gateway',
            get_string('gateway', 'block_coursesentimentgateway_aws'),
            $gateways,
        );
        $mform->setDefault(
            elementName: 'gateway',
        ...
        $mform->addElement(
            'checkbox',
            'usecredchain',
            get_string('usecredchain', 'block_coursesentimentgateway_aws'),
            ' ',
        );
        $mform->setDefault(
        ...
        $mform->addElement(
            'text',
            'api_key',
            get_string('api_key', 'block_coursesentimentgateway_aws'),
        ...
        $mform->addElement(
            'passwordunmask',
            'api_secret',
            get_string('api_secret', 'block_coursesentimentgateway_aws'),
            'maxlength="255" size="20"',
        );
        ...
        $mform->addElement(
            'text',
            'api_region',
            get_string('api_region', 'block_coursesentimentgateway_aws'),
            'maxlength="255" size="20"',
        );
